---
seo:
  title: "Loïc Lavanchy, Berater für Automatisierung"
  description: "Loïc Lavanchy begleitet KMU, Selbständige und Startups bei Prozessautomatisierung mit n8n und beim sinnvollen Einsatz von KI."
hero:
  eyebrow: "Über mich"
  headline: "Wer dahintersteckt"
  subheadline: "Ich verbinde Fachbereiche und IT. Mein Fokus liegt auf verständlichen Lösungen, die im Alltag funktionieren und Schritt für Schritt mit dem Unternehmen wachsen."
werdegang:
  headline: "Vom Requirement Engineering zur Selbständigkeit"
  text:
    - "Als Projektleiter und Requirement Engineer habe ich in den letzten neun Jahren Fachbereiche und IT zusammengebracht. Dabei habe ich beim Institut für Geistiges Eigentum auf Bundesebene und bei einem Schweizer Software Unternehmen gearbeitet."
    - "Meine Aufgabe war es, die Bedürfnisse der Fachbereiche zu verstehen und daraus Lösungen zu entwickeln, die im Alltag tatsächlich funktionieren. Davon habe ich vier Jahre mit besonderem Fokus auf Prozesse gearbeitet."
    - "Unter anderem habe ich ein papierbasiertes Verfahren mit rund 7'000 Verfahren pro Jahr digitalisiert und automatisiert. Das Ergebnis waren rund 90 Prozent weniger manueller Aufwand pro Verfahren und mehr als 40 Prozent mehr bearbeitete Fälle innerhalb eines Jahres."
    - "Mich interessiert, wie Dinge funktionieren. Wenn ich einen Ablauf kennenlerne, will ich zuerst verstehen, warum er so gewachsen ist. Meistens steckt ein guter Grund dahinter. Und oft zeigt sich beim genauen Hinschauen, dass ein Teil davon einfacher gehen könnte."
    - "2020 bin ich zum ersten Mal mit Camunda in Berührung gekommen, einer Plattform zur Prozessautomatisierung. Der Ansatz hat mich überzeugt und gleichzeitig enttäuscht: Für kleinere Vorhaben war das Werkzeug schlicht zu schwer. Ich konnte es selbst nicht einsetzen."
    - "Mit n8n hat sich das aufgelöst. Plötzlich liess sich in Tagen umsetzen, wofür es vorher ein ganzes Projekt gebraucht hätte. Genau deshalb habe ich mich selbständig gemacht."
    - "Heute begleite ich KMU und Selbständige dabei, administrative Arbeit zu reduzieren und mehr Zeit für Aufgaben zu gewinnen, die einen echten Mehrwert bringen."
wieIchArbeite:
  headline: "Zuhören, verstehen und gemeinsam weitergehen"
  text:
    - "Bevor ich etwas baue, will ich verstehen, wie heute gearbeitet wird und was euch wirklich beschäftigt. Dieses Zuhören begleitet mich durch den ganzen Prozess."
    - "Eure Rückmeldungen fliessen laufend in die Lösung ein. Technische Entscheidungen werden verständlich erklärt und gemeinsam besprochen."
    - "Mir ist ein pragmatischer Ansatz wichtig. Ziel ist eine Lösung, die schnell einen echten Nutzen bringt und nicht unnötig kompliziert ist."
    - "Wer weniger Zeit mit Aufgaben verbringt, die als Pflicht empfunden werden und viel Energie kosten, kann sich stärker auf Aufgaben konzentrieren, die Freude machen und einen Mehrwert schaffen. Das ist für einzelne Personen motivierender und für Unternehmen ein echter Gewinn."
  grundsaetze:
    - title: "Bestehendes nutzen"
      text: "Eure Tools werden nicht ersetzt, sondern sinnvoll miteinander verbunden."
    - title: "Bedürfnisse verstehen"
      text: "Die Lösung soll zum Alltag und zu den Menschen passen, die damit arbeiten."
    - title: "Verständlich erklären"
      text: "Ihr sollt verstehen, was umgesetzt wird und warum."
    - title: "Gemeinsam iterieren"
      text: "Die Lösung wird mit euren Rückmeldungen weiterentwickelt."
wieIchVorgehe:
  headline: "Pragmatisch bauen, sauber testen"
  text: "Die technische Umsetzung folgt einem klaren Ablauf:"
  schritte:
    - title: "Pragmatisches Konzept erstellen"
      text: "Ich entwickle eine Lösung, die zum heutigen Alltag passt."
    - title: "Umsetzung mit KI Unterstützung"
      text: "Ich baue die Automatisierung mit n8n und nutze KI Werkzeuge dort, wo sie die Umsetzung sinnvoll unterstützt."
    - title: "Ergebnis testen"
      text: "Die Automatisierung wird geprüft, bevor sie produktiv eingesetzt wird."
    - title: "Ergebnis besprechen und anpassen"
      text: "Wir besprechen das Ergebnis und passen die Lösung an, bis sie im Alltag funktioniert."
    - title: "Automatisierte Tests und Fehlerbehandlung"
      text: "Wo es sinnvoll ist, werden automatische Tests und Fehlerbehandlungen eingebaut. Das verbessert Wartbarkeit und Nachvollziehbarkeit."
    - title: "Prozessweise produktiv machen"
      text: "Die Umsetzung erfolgt Prozess für Prozess. Der nächste Prozess wird erst angegangen, wenn der vorherige produktiv ist, so wächst die Automatisierung Schritt für Schritt mit euch mit."
  abschlussText: "Ihr seht jederzeit, was automatisiert läuft und wie der Prozess aufgebaut ist. So entsteht keine Blackbox, sondern eine nachvollziehbare Lösung."
testimonials:
  - name: "Xenia Abdin"
    quote: "Ich habe die Zusammenarbeit mit Loïc in verschiedenen bereichsübergreifenden Projekten als sehr angenehm erlebt. Er hat sich stets die Zeit genommen, die Anforderungen und Bedürfnisse der Fachbereiche genau zu verstehen, bevor er Lösungen umgesetzt hat. Seine Offenheit für Rückmeldungen und sein Engagement, praxistaugliche Lösungen zu entwickeln, haben die Zusammenarbeit wesentlich erleichtert."
    role: "Kundin"
  - name: "Elie Jeannerat"
    quote: "Loïc war stets ein pragmatischer Projektleiter und Business Analyst. Er hörte seinen Kunden und Kollegen zu, um die Bedürfnisse aller Beteiligten zu verstehen und die besten Kompromisse zu finden. Der Mensch steht im Zentrum seines Ansatzes."
    translationNote: "Aus dem Französischen übersetzt."
    role: "Software Engineer"
  - name: "Patrick Andreas Näf"
    quote: "Kann die Arbeit von Loïc sehr empfehlen. Besonders schätze ich seine Kompetenz, auf Menschen zuzugehen und Probleme in Projekten proaktiv anzusprechen. Seine Einsatzbereitschaft ist hervorragend."
    role: "ehemaliger Vorgesetzter"
  - name: "Mihail Leontescu"
    quote: "Die Zusammenarbeit mit Loïc war eine grossartige Erfahrung. Sein Beitrag im Requirement Engineering Team war entscheidend dafür, digitale Lösungen zu liefern, die unsere Abteilung produktiv einsetzen konnte. Besonders geschätzt habe ich seinen konstruktiven und pragmatischen Ansatz."
    translationNote: "Aus dem Englischen übersetzt."
    role: "Digital Services Operations"
linkedinNote: "Die vollständigen Empfehlungen können auf LinkedIn eingesehen werden."
persoenliches:
  headline: "Kurz gesagt"
  intro: "Meine Wurzeln liegen in Lausanne, aufgewachsen bin ich in Baden, studiert habe ich in Olten und gearbeitet habe ich in Bern. Dadurch bin ich im grössten Teil der Schweiz zuhause."
  items:
    - "Standort: St-Sulpice VD, Schweiz"
    - "Sprachen: Deutsch und Französisch, Englisch"
    - "Erfahrung: Mehr als neun Jahre Requirement Engineering und IT Projektleitung, davon vier Jahre mit Fokus auf Prozesse"
    - "Antrieb: Meinen Kunden und mir mehr Flexibilität und mehr Zeit für Aufgaben ermöglichen, die einen echten Mehrwert bringen"
  nebenDerArbeit:
    - "Reisen ist eine meiner Leidenschaften. Dabei geht es mir darum, den Horizont zu erweitern: neue Orte, neue Menschen, neue Sichtweisen."
    - "Als Ausgleich lese ich, gehe wandern oder mache Wassersport. Hoch im Kurs stehen die Bücher von Clive Cussler. Die Neugier, die mich unterwegs antreibt, ist dieselbe, die mich bei Prozessen fragen lässt, wie sich etwas einfacher lösen liesse."
abschlussCta:
  headline: "Bereit, eure Prozesse zu vereinfachen?"
  text: "In einem kostenlosen Erstgespräch schauen wir gemeinsam auf einen eurer Abläufe. Unverbindlich, auf Deutsch, Französisch oder Englisch."
  buttonLabel: "Kostenloses Erstgespräch buchen"
---
